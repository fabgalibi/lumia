...
import TrajectoryStudyCard from './trajectory-study-card';
...
interface TrajectorySelectionContentProps {
  trajectoryData: TrajectoryData;
  onTrajectoryDataChange: (data: Partial<TrajectoryData>) => void;
  screenSize: ScreenSize;
  showTitles?: boolean;
  showWorkStatusSection?: boolean;
}

// Dados dos cards de tempo de estudo conforme Figma (dados originais do profile-setup)
...
export const TrajectorySelectionContent: React.FC<TrajectorySelectionContentProps> = ({
  trajectoryData,
  onTrajectoryDataChange,
  screenSize,
  showTitles = true,
  showWorkStatusSection = true
}) => {
  const { studyTime, isWorking } = trajectoryData;

  const handleStudyTimeChange = (timeId: string) => {
    onTrajectoryDataChange({ studyTime: timeId });
  };

  const handleWorkStatusChange = (status: string) => {
    onTrajectoryDataChange({ isWorking: status });
  };
  ...
  return (
    <>
      ...
      {/* Segunda Seção - Status de Trabalho (condicionada pela flag) */}
      {showWorkStatusSection && (
        ...
        >
          ...
          >
            {/* Opção Sim */}
            <div
              onClick={() => handleWorkStatusChange('sim')}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '12px',
            ...
            {/* Opção Não */}
            <div
              onClick={() => handleWorkStatusChange('nao')}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '12px',
